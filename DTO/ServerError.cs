// ------------------------------------------------------------------------------
// <auto-generated>
//    Generated by avrogen.exe, version 0.9.0.0
//    Changes to this file may cause incorrect behavior and will be lost if code
//    is regenerated
// </auto-generated>
// ------------------------------------------------------------------------------
namespace HelloAvro.DTO
{
	using System;
	using System.Collections.Generic;
	using System.Text;
	using Avro;
	using Avro.Specific;
	
	public partial class ServerError : SpecificException
	{
		public static Schema _SCHEMA = Avro.Schema.Parse("{\"type\":\"error\",\"name\":\"ServerError\",\"namespace\":\"HelloAvro.DTO\",\"fields\":[{\"name" +
				"\":\"message\",\"type\":\"string\"}]}");
		private string _message;
		public override Schema Schema
		{
			get
			{
				return ServerError._SCHEMA;
			}
		}
		public string message
		{
			get
			{
				return this._message;
			}
			set
			{
				this._message = value;
			}
		}
		public override object Get(int fieldPos)
		{
			switch (fieldPos)
			{
			case 0: return this.message;
			default: throw new AvroRuntimeException("Bad index " + fieldPos + " in Get()");
			};
		}
		public override void Put(int fieldPos, object fieldValue)
		{
			switch (fieldPos)
			{
			case 0: this.message = (System.String)fieldValue; break;
			default: throw new AvroRuntimeException("Bad index " + fieldPos + " in Put()");
			};
		}
	}
}
